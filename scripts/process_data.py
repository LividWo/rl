...
class LlamaAgent():
    ...
    def get_log_prob(self, observation, action):
        ...
        # 尽管这里的 output 毫无意义
        # 但我们要的是，和 input 中的 action 相同位置上，output 的每一个 action token 对应的概率
        ...
        # 扔掉原来 output 的无意义概率
        # 换成 action token 对应的概率
        log_prob = action_log_probs * action_attention_mask
        res_log_prob = []
        # 如果 action_idx = self.func(action[i])的话
        # 截取 实际 action 的概率，去掉前面可能有的说明
        # 然而这里的 action_idx = 0
        # 所以 res_log_prob == log_prob
...
def trace_process(dir_path):
    ...
    for trace in tqdm(traces_paths, ascii=True):
        ...
        # 如果 fixed_traces 中已有本次 trace，则不处理
...
def process_data_chunk(data_chunk, actor_path, device_id, result_queue):
    torch.cuda.empty_cache()
    # Assign GPUs for actor 
    actor_device = f'cuda:{device_id}'
    ...
    output_file = f'{device_id}.pt'
    torch.save(all_items, output_file)

def replay_data(files, actor_path, output_path, num_gpu: int):
    # files 是一堆 .pt 文件，内部是 list of list
    file_contents = []
    for file in files:
        ...
        for traj in file_content:
            if isinstance(traj, list) == True:
                ...
                print(file)
    # file content 是 list of list，存了所有 trajectory
    # file_chunks 是把 file content 切成了 num_gpu 份
    file_chunks = [file_contents[i::num_gpu] for i in range(num_gpu)]
    ...
    gpu_pairs = [i for i in range(num_gpu)]
    # Start processes
    ...
    # current_data 就是 D_rollout
    current_data = torch.load(output_path)
    ...
    print(output_path.replace('.pt', '_filter.pt'))
    torch.save(all_data, output_path.replace('.pt', '_filter.pt'))
...
if __name__ == '__main__':
    ...
    if '1' in args.stage:
        # 把 result/traces 中的内容拿出来，其每一行是一个字典，称为 step
        # 为 step 添加域 ‘fixed_response'，其值为 step['response']
        # 为 step 添加域 ‘score'，其值为本测例的最终得分
        # 把更新后的 trace 存到 result/fixed_traces 中
        trace_process(args.rollout_path)
        # 调用 orm，为 fixed_traces 生成由模型判定的 reward，使用这一 reward 覆盖原有的 ‘score’ 域的值
        # 如果指定了 output_path, 就把 orm 的输入 prompt 和输出保存下来
        build_rm_data(args.rollout_path, None, args.add_reward, args.orm_path)
        # 把 fixed_traces 变成如下数据
        # {
        # 'observation': 见下方,
        # 'next_observation': 见下方,
        # 'task': task, 
        # 'reward': trace_content[-1]['score'], 
        # 'done': False if i != len(trace_content) - 1 else True, 
        # 'action': item['fixed_response'], 
        # 'trajectory_reward': 1 if trace_content[-1]['score'] >= 0.5 else 0
        # }
        # 最后把所有 trace 打包存到 output_path 中（是单独的文件）
        # ‘observation': 
        ...
        # <|eot_id|><|start_header_id|>assistant<|end_header_id|>
        # "
        # 'next_observation': 就是在 observation 的基础上多了一步
        build_policy_data(args.rollout_path, args.output_path)
    if '2' in args.stage:
        replay_data(args.experience_paths, args.actor_path, args.output_path, args.num_gpu)