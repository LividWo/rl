...
class VLMDoubleCritic(nn.Module):
    def __init__(self, device, accelerator, critic_lm, cache_dir, in_dim, out_dim):
        ...
        self.base_lm = AutoModelForCausalLM.from_pretrained(critic_lm, use_cache=False, trust_remote_code=True, low_cpu_mem_usage=True).model.to(f'cuda:{current_rank}')
        ...
        self.critic1 = nn.Sequential(nn.Linear(in_dim, in_dim),\
                                    ...
                                    nn.ReLU(),\
                                    nn.Linear(in_dim, out_dim)).to(f'cuda:{current_rank}')
    ...
    def forward(self, observation, action, detach_model=False):
        # detach_model = True
        obs_ids = self.base_tokenizer(observation, padding = True, return_tensors='pt', truncation = True).to(self.base_lm.device)
